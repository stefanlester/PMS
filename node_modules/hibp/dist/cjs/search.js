...
var breachedAccount = require('./breachedAccount.js');
var pasteAccount = require('./pasteAccount.js');
...
function search(account) {
  ...
  var apiKey = breachOptions.apiKey,
      baseUrl = breachOptions.baseUrl,
      userAgent = breachOptions.userAgent;
  return Promise.all([breachedAccount.breachedAccount(account, breachOptions), // This email regex is garbage but it seems to be what the API uses:
  /^.+@.+$/.test(account) ? pasteAccount.pasteAccount(account, {
    apiKey: apiKey,
    baseUrl: baseUrl,
    userAgent: userAgent
  }) : null]).then( // Avoid array destructuring here to prevent need for Babel helpers
  function (promises) {
    return {
      breaches: promises[0],
      pastes: promises[1]
